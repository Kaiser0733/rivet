...
class WorkspaceSearchTest {
    ...
    @Test fun binaryAndInaccessibleFilesAreSkipped() = runTest {
        val result = searchWorkspace("hit", entry("", true, "root"), SearchLimits(),
    ...
    @Test fun cancellationIsNotSwallowed() = runTest {
        try {
            searchWorkspace("x", entry("", true, "root"), SearchLimits(),
    ...
    @Test fun directoryAliasesCannotRecurseForever() = runTest {
        var listings = 0
        val result = searchWorkspace("x", entry("", true, "root"), SearchLimits(maxEntries = 5),
    ...
    @Test fun unknownLengthOversizeReadsStayWithinTotalBudget() = runTest {
        var consumed = 0
        val result = searchWorkspace("none", entry("", true, "root"),
    ...
    @Test fun oversizedListingExhaustsTheEntryBudget() = runTest {
        var listings = 0
        val result = searchWorkspace("none", entry("", true, "root"), SearchLimits(maxEntries = 3),
            { _, _ -> listings++; throw WorkspaceFailure(WorkspaceFailure.Reason.LIMIT) },
            { _, _ -> byteArrayOf() })
        assertEquals(1, listings)
        assertEquals(3, result.entriesVisited)
        assertTrue(result.limited)
    }
    @Test fun fileBudgetStopsWithoutDependingOnResultLimit() = runTest {
        var reads = 0
        val result = searchWorkspace("absent", entry("", true, "root"), SearchLimits(maxFiles = 2),
            { _, _ -> (1..5).map { entry("$it") } }, { _, _ -> reads++; byteArrayOf() })
